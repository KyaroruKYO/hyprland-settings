...
- Runtime/reload read-only evidence retry found `/usr/bin/hyprctl`, a Hyprland instance signature, and a socket directory under `$XDG_RUNTIME_DIR/hypr`, but `hyprctl version`, `hyprctl monitors -j`, and `hyprctl getoption general:gaps_in` all failed without mutation with `Couldn't set socket timeout (2)`.
...
GTK evidence root: `/tmp/hyprland-settings-gtk-automation/20260619_194620`.
