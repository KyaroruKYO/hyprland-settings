...
## Backend Completion Marathon (2026-07-14)

Moved the app from "133 editable scalar rows" to **219 editable** by adding two new pending-ledger entry kinds, both GUI-proven live, while keeping every gate intact.

- **Save-only pending ledger (Scope B)** — `src/save_only_pending.rs`: rows classified `StatusChip::SaveOnly` (48 rows: misc/binds/gestures flags, kb layout, fonts, regexes) now get an inline control that **stages a config draft** instead of writing immediately. The draft joins the unified pending surfaces (row accent, sidebar count, header chip, bottom bar, Pending Changes page + diff) and Save persists it through the existing `gated_scalar_save_live`. **No runtime mutation happens before Save** — proven live: toggling `binds:pass_mouse_when_bound` left `getoption` at `bool: false` while all pending surfaces lit up; the diff wrote `+binds:pass_mouse_when_bound = true` to the discovered `~/.config/hypr/hyprland.conf`; Discard cleared it. The classifier admits exactly the 48 `SaveOnly` rows and rejects every high-risk/hardware/dead-man/not-proven/live-preview row (`tests/save_only_pending.rs`, 8 guards).
- **Dead-man rollback modal (Scope D)** — `open_dead_man_rollback_dialog` in `src/ui/window.rs`: an in-window `adw::Dialog` (not a tiled window) that appears once a supervised change is applied live, counts down from **15 seconds** ("Reverting to … in N seconds."), and offers Keep changes / Revert. Revert is the default and fires on timeout/Escape/close; Keep confirms and the kept change joins the pending ledger via the new `PendingSource::DeadManKept`, so Save persists it and Discard reverts it through the controller. Countdown is bound to `RUNTIME_PREVIEW_DEAD_MAN_TIMEOUT_MS` (raised 10→15s) so display and real revert can't drift. Proven live on `input.repeat_rate`: `25` → armed applies `0` → "15 seconds" then "9 seconds" → Revert restored `25`; Keep held `0` and showed pending; Discard restored `25`. `tests/dead_man_rollback_dialog.rs` (9 guards) + `tests/backend_completion_matrix.rs` (partition guard, all 341 rows in exactly one bucket).
- **Unified ledger**: `pending_change_snapshots()` merges LivePreview + SaveOnlyDraft + DeadManKept; the bottom bar status is now source-aware ("applied live" only when something actually is, else "staged").
- **Still blocked, honestly**: 82 high-risk (incl. monitor — read-only, no proven display rollback), 27 hardware-gated, 11 not-proven scrolling/layout rows, 2 upstream-readback-broken font_weight rows; profile save/apply stays disabled (no proven whole-config rollback); style editing stays blocked (installed Hyprland ships only protocol headers + a Lua stub with no style enumeration). Reports: `data/reports/backend-completion-*.json`, `high-risk-recovery-backend`, `structured-family-backend-completion`, `profiles-modes-config-targets`, `source-aware-config-backend`, `style-editing-evidence`. Screenshots in `/home/kyo/review-backend-completion-after/`.

## Editability Completion Pass (2026-07-14)

Broadened save-only editability from 48 rows to **117**, taking editable scalar rows from **219 → 290**. The key reframing: save-only is a *deferred, gated, backed-up, reread-verified config write that never touches the running session*, so the high-risk/hardware/not-proven labels (which were about live preview) do not block it. `is_save_only_editable` now admits any row that is not live-previewable, not armable under the dead-man countdown, has a concrete control, has a gated write path, and is **not production-gated**. `save_only_control_kind` gained Entry mappings for Path/MonitorName/Vector2/NumericList/CommaSeparatedFloatList/AccelProfile/CssGap/StringLike (validated at gated save).

- **The one hard stop**: 51 settings are in the accepted high-risk production-gate lists (display/render, cursor/input, debug/crash). `apply_setting_change` **rejects** them without a `HighRiskProductionGate` recovery-plan proof, which the plain save-only path cannot supply. Exposing a control would be editable-but-unsavable, and bypassing the gate is forbidden — so they stay blocked deliberately (`is_high_risk_gated_writable_setting` excludes them). Proven live: `render.direct_scanout` and `opengl.nvidia_anti_flicker` render **no** inline control.
- **Proven live per newly-unlocked category** (`/home/kyo/review-full-editability-after/`): hardware-gated `input.touchpad.natural_scroll` and not-proven `scrolling.wrap_focus` now stage pending with no runtime mutation and discard cleanly; guard `tests/save_only_pending.rs::every_non_gated_non_live_row_with_a_control_is_editable` proves all 117 have the control + path with no gaps.
- **Final scalar matrix**: 341 visible, **290 editable** (135 live-preview + 117 save-only + 38 dead-man), 51 production-gated blocked, 0 blocked for any other reason, 2 upstream-readback-broken but savable. Reports `full-editability-*.json`.
- **NOT done this pass (honest)**: the 51 production-gated rows (need the recovery-plan flow, a safety-critical subsystem); the 5 structured-family editors (hl.monitor/bind/gesture/device/permission — each a real editor with grammar/validation/write-reread); functional profiles; style editing. All reported with concrete proof requirements, not faked. Follow-up: the status chip for now-editable rows still reads "Hardware required"/"Blocked" (stale — describes live preview, not the save-only editor).
