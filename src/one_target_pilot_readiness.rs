...
pub struct OneTargetPilotReadinessMapping {
    ...
    pub advanced_confirmation_risk_policy_complete: bool,
...
impl OneTargetPilotReadinessMapping {
    pub fn is_ready_for_production(&self) -> bool {
        self.backup_contract_complete
            ...
            && self.advanced_confirmation_risk_policy_complete
    ...
    pub fn user_facing_lines(&self) -> Vec<String> {
        vec![
            ...
            "If recovery fails, the app will report the failure and leave the backup available."
                .to_string(),
            "Recovery contract approval is staged; recovery execution stays limited to guarded safe-batch failure handling.".to_string(),
            "Advanced confirmation".to_string(),
            "Some files may be changed by scripts, generated by tools, or managed through symlinks."
                .to_string(),
            "These targets remain blocked from safe-batch writing.".to_string(),
            ...
            "High-risk rows remain blocked from safe-batch writing.".to_string(),
            "High-risk approval is not active yet.".to_string(),
            "Final safe-batch readiness".to_string(),
            "Safe-batch writing is available only for eligible normal scalar settings.".to_string(),
            "The pre-enable audit stage is complete.".to_string(),
            "Non-writing prerequisite gates are approved.".to_string(),
            "Write execution stays guarded by safe-batch eligibility checks.".to_string(),
            "Backup creation is allowed only inside the guarded safe-batch write path.".to_string(),
            "Verification execution is allowed only inside the guarded safe-batch write path.".to_string(),
            "Target review and target selection are approved only for safe-batch guarded paths.".to_string(),
            "Unsafe writes remain blocked.".to_string(),
            "Apply remains blocked for missing/default, duplicate, high-risk, display/render, generated, script-managed, symlink/current-profile, structured, runtime, and profile/mode targets.".to_string(),
        ]
    }
}
...
pub fn current_one_target_pilot_readiness_mapping() -> OneTargetPilotReadinessMapping {
    OneTargetPilotReadinessMapping {
        ...
        advanced_confirmation_risk_policy_complete: false,
