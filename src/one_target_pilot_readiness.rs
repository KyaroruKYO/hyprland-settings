    ...
    pub fn user_facing_lines(&self) -> Vec<String> {
        vec![
            ...
            "If recovery fails, the app will report the failure and leave the backup available."
                .to_string(),
            "Recovery contract approval is staged; recovery execution is still blocked until the pilot is approved.".to_string(),
            ...
            "Write execution gates are still disabled.".to_string(),
            "Backup contract approval is staged; backup creation is still blocked until write execution gates are approved.".to_string(),
            "Verification contract approval is staged; verification execution is still blocked until write execution gates are approved.".to_string(),
            "Target review and target selection are approved as prerequisites only.".to_string(),
