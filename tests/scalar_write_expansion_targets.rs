...
fn parser_needed_target_research_decisions_are_recorded() -> Result<()> {
    ...
    assert_eq!(decisions.get("manual-review-needed"), Some(&21));
...
fn scalar_write_expansion_target_report_has_expected_counts() -> Result<()> {
    ...
    assert_eq!(report["counts"]["parserNeededTargetsEnabled"], 16);
    assert_eq!(report["counts"]["parserNeededTargetsBlocked"], 21);
