...
#[test]
fn advanced_confirmation_and_write_gates_remain_false() {
...
#[test]
fn write_flow_does_not_import_or_call_advanced_confirmation_policy() {
    let write_flow =
        fs::read_to_string("src/write_flow.rs").expect("write flow source should read");

    for forbidden in [
        "production_advanced_confirmation",
        "TargetManagementRisk",
        "production_advanced_confirmation_policy",
        "classify_target_management_risk",
        "recommendation_risk_explanation_for_candidate",
