...
#[test]
fn visual_review_sprint_keeps_write_activation_gates_false_after_pre_enable_approval() {
...
#[test]
fn write_flow_does_not_import_or_call_visual_review_or_gate_proposal_models() {
    let write_flow =
        fs::read_to_string("src/write_flow.rs").expect("write flow source should read");

    for forbidden in [
        "one_target_pilot_live_visual_smoke",
        "one_target_pilot_live_visual_smoke_review_result",
        "one_target_pilot_gate_flip_proposal_draft",
        "one_target_pilot_visual_gate_flip_proposal_readiness",
        "one_target_pilot_visual_review_remaining_blockers",
        "disabled_live_visual_smoke_review_ui_lines",
